...
from src.logreg_trainer import LogRegTrainer  # LogRegTrainer는 우회하더라도 임포트는 유지합니다.
...
def run_analysis():
    ...
    # LogReg용 데이터 (10개 등급) - 현재 오류로 인해 사용하지 않음
    ...
    # LogReg 단계는 현재 오류가 지속되어 건너뛰고, 1차 선정된 VIF 변수로 바로 진행합니다.
    print("\n!! [조치] statsmodels 라이브러리 오류로 인해 LogReg 2차 변수 선정은 생략하고,")
    print("!! 1차 선정된 VIF 변수(총 18개)로 최종 Random Forest 튜닝을 진행합니다.")
    ...
    # 튜닝 범위 (n_estimators와 max_features 위주로 튜닝)
    param_grid = {
        'n_estimators': [100, 150, 200, 250, 300],
        'max_features': [0.5, 0.7]  # 예: 전체 특징 중 50% 또는 70% 사용
    }

    rf_trainer = RFTrainer()
    # RF는 3개 그룹(y_train_agg)을 사용하며, 2차 선정된 X_train_final을 사용합니다.
    ...
    # 3개 그룹의 레이블 이름 정의
    target_names = ['고위험(0)', '중위험(1)', '저위험(2)']
